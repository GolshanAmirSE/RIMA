...
- **Install a new player version (from a new zip):** the zip never contains `music/library.json`, so your song list is safe. Unzip it and copy everything into your `RIMA` folder, choosing **Replace** (GitHub Desktop) — or upload those files on the website. Keep your own `music` folder, so your songs and `library.json` stay. Then commit and push. Old files in `assets` can stay; they do no harm.
